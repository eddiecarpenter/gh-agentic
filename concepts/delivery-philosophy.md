...
`.agents/concepts/feature-switches.md` for the full taxonomy.
...
A GitHub Actions workflow in `.agents/.github/workflows/release.yml` triggers on any
`v*` tag push:
...
The framework provides an example at `.agents/docs/examples/publish-release.yml`.
...
See `.agents/RULEBOOK.md` → Testing → Integration Tests for the enforcement rules.
...
See `.agents/concepts/feature-switches.md` for the full taxonomy, modes, and lifecycle.
