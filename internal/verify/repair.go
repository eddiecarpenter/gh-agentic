...
// store. On macOS (runtime.GOOS == "darwin"), it tries the file first, then
// falls back to the macOS Keychain. On Linux/Windows, it reads from the file.
// Returns the raw credential bytes or an error if not found.
func ReadClaudeCredentials(run bootstrap.RunCommandFunc, readFile func(string) ([]byte, error), homeDir func() (string, error)) ([]byte, error) {
	home, err := homeDir()
	if err != nil {
...
func ReadClaudeCredentialsDefault(run bootstrap.RunCommandFunc) ([]byte, error) {
...
func RepairClaudeCredentialsSecretWithReadFile(owner, repoName, ownerType string, run bootstrap.RunCommandFunc, readFile func(string) ([]byte, error), homeDir func() (string, error)) CheckResult {
	data, err := ReadClaudeCredentials(run, readFile, homeDir)
	if err != nil {
