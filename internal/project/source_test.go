...
func TestIsFrameworkSource(t *testing.T) {
	...
	t.Run(".ai is a regular directory — consumer after mount", func(t *testing.T) {
		root := t.TempDir()
		if err := os.Mkdir(filepath.Join(root, ".ai"), 0o755); err != nil {
	...
	t.Run(".ai is a symlink — framework source", func(t *testing.T) {
		root := t.TempDir()
		...
		if !IsFrameworkSource(root) {
			t.Fatal("expected true when .ai is a symlink")
		}
	})

	t.Run(".ai is a regular file — unexpected but must not false-positive", func(t *testing.T) {
		root := t.TempDir()
		if err := os.WriteFile(filepath.Join(root, ".ai"), []byte("x"), 0o644); err != nil {
			t.Fatalf("setup: %v", err)
		}
		if IsFrameworkSource(root) {
