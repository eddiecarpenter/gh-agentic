...
- [`docs/migration-cp-centralized.md`](docs/migration-cp-centralized.md) — CP-side runbook for moving a pre-pivot federation (cross-repo sub-issues, domain-repo mounts) onto the control-plane-centralized model under requirement #870 (features on the CP with a Target-repo field, same-repo sub-issues, pure-code domain repos). A federation created on the current framework is already CP-centralized — the runbook documents the no-op path.
